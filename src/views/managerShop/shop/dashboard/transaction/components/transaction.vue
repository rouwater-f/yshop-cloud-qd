   ...
   <div class="divBox" style="padding-top:0;">
      <el-card class="box-card">
        <div class="flex justify-between mb46">
          <div class="header_title">交易状况</div>
          <div class="acea-row">
            <div class="mr-20">
              <el-radio-group v-model="dateLimit" type="button" class="mr20" size="small" @change="selectChange(dateLimit)">
              </el-radio-group>
              <el-date-picker
              v-model="timeVal"
              size="small"
          ...
          <div class="infoBox acea-row mb30" v-for="(item, index) in list" :key="index">
            ...
            <div class="info">
              ...
              <span class="content-time"><i :class="item.ratio >= 0 ? 'up' : 'down'">{{ item.list.percent }}</i>
                <i :color="item.ratio >= 0 ? '#F5222D' : '#39C15B'" :class="item.ratio >= 0 ? 'el-icon-caret-top' : 'el-icon-caret-bottom'"></i>
              </span>
            </div>
          </div>
        </div>
        <echarts-new :option-data="optionData" :styles="style" height="100%" width="100%"></echarts-new>
      </el-card>
    </div>
</template>

<script>
import { tradeOverviewApi,tradeTrendApi} from '@/api/dashboard'
